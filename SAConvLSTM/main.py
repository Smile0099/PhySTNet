...
class Trainer:
    ...
    def train_once(self, input_sst, sst_true, ssr_ratio):
        ...
        loss_new3 = self.loss(sst_pred, sst_true.float().to(self.device), self.new3)
        loss = loss_z500+loss_t850+loss_t2m+loss_new+loss_new2+loss_new3
        loss.backward()
        if configs.gradient_clipping:
            nn.utils.clip_grad_norm_(self.network.parameters(), configs.clipping_threshold)
        self.opt.step()
        return loss_z500.item(), loss_t850.item(), loss_t2m.item(), loss_new.item(),loss_new2.item(),loss_new3.item(), loss
    ...
    def infer(self, dataset, dataloader):
        ...
        with torch.no_grad():
            ...
            loss_new3 = self.loss(sst_pred, sst_true, self.new3).item()

        return loss_z500, loss_t850, loss_t2m, loss_new, loss_new2, loss_new3
    ...
    def train(self, dataset_train, dataset_eval, chk_path):
        ...
        for i in range(self.configs.num_epochs):
            ...
            for j, (input_sst, sst_true) in enumerate(dataloader_train):
                ...
                loss_z500, loss_t850, loss_t2m, loss_new, loss_new2, loss_new3, loss  = self.train_once(input_sst, sst_true, ssr_ratio)  # y_pred for one batch

                if (j+1) % self.configs.display_interval == 0:
                    print('batch training loss: {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f},{:.2f}, {:.2f}, ssr: {:.5f}'.format(loss_z500, loss_t850, loss_t2m, loss_new,  loss_new2, loss_new3,loss, ssr_ratio))
                ...
                if (i+1 >= 6) and (j+1)%(self.configs.display_interval * 2) == 0:
                    loss_z500_eval_0, loss_t850_eval_0, loss_t2m_eval_0, loss_new_eval_0, loss_new2_eval_0, loss_new3_eval_0 = self.infer(dataset=dataset_eval, dataloader=dataloader_eval)
                    loss_eval_0 = loss_z500_eval_0 + loss_t850_eval_0 + loss_t2m_eval_0 + loss_new_eval_0 + loss_new2_eval_0 + loss_new3_eval_0
                    print('batch eval loss: {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}'.format(loss_z500_eval_0, loss_t850_eval_0, loss_t2m_eval_0, loss_new_eval_0,loss_new2_eval_0,loss_new3_eval_0, loss_eval_0))
            ...
            # evaluation
            loss_z500_eval, loss_t850_eval, loss_t2m_eval, loss_new_eval, loss_new2_eval, loss_new3_eval = self.infer(dataset=dataset_eval, dataloader=dataloader_eval)
            loss_eval = loss_z500_eval + loss_t850_eval + loss_t2m_eval + loss_new_eval + loss_new2_eval + loss_new3_eval
            print('epoch eval loss: {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}'.format(loss_z500_eval, loss_t850_eval, loss_t2m_eval, loss_new_eval,  loss_new2_eval, loss_new3_eval,loss_eval))
...
if __name__ == '__main__':
    ...
    train_x , train_y, test_x , test_y, mean, std = data['train_x'],data['train_y'],data['test_x'],data['test_y'],data['mean'],data['std']
    #     # train_x = np.concatenate([data_x[:8406],data_y[:8406]],1)
    #     # train_y = data_y[:8406]
    #     # test_x = np.concatenate([data_x[8406:],data_y[8406:]],1)
    #     # test_y = data_y[8406:]
    #     #
    train_x = np.concatenate([train_x ,train_y],1)
...
#     trainer.train(dataset_train, dataset_test, 'checkpoint')
    ...
    np.savez('result135.33531856536865.npz', pred=pred.cpu(), label=label.cpu())