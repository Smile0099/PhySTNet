...
class Trainer:
    def __init__(self, configs):
        ...
        torch.manual_seed(5)
        self.network = SimVP_Model(configs).to(configs.device)
        ...
        for i in range(self.configs.num_epochs):
            ...
            for j, (input_sst, sst_true) in enumerate(dataloader_train):
                ...
                if (i+1 >= 6) and (j+1)%(self.configs.display_interval * 2) == 0:
                    ...
                    print('batch eval loss: {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}'.format(
                        loss_z500_eval_0, loss_t850_eval_0, loss_t2m_eval_0, loss_new_eval_0, loss_new2_eval_0,
                        loss_new3_eval_0, loss_eval_0))
            ...
            # evaluation
            # evaluation
            loss_z500_eval, loss_t850_eval, loss_t2m_eval, loss_new_eval, loss_new2_eval, loss_new3_eval = self.infer(
                dataset=dataset_eval, dataloader=dataloader_eval)
            loss_eval = loss_z500_eval + loss_t850_eval + loss_t2m_eval + loss_new_eval + loss_new2_eval + loss_new3_eval
            print('epoch eval loss: {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}, {:.2f}'.format(loss_z500_eval,
                                                                                                   loss_t850_eval,
                                                                                                   loss_t2m_eval,
                                                                                                   loss_new_eval,
                                                                                                   loss_new2_eval,
...
if __name__ == '__main__':
    ...
    net = torch.load('checkpoint_65.79461812973022.chk')
    ...
    np.savez('result65.79461812973022.npz', pred=pred.cpu(), label=label.cpu())
