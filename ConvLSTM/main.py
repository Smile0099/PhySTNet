...
class Trainer:
    def __init__(self, configs):
        ...
        torch.manual_seed(5)
        self.network = ConvLSTM_Model(configs).to(configs.device)
    ...
    def train_once(self, input_sst, sst_true, ssr_ratio):
        ...
        self.opt.zero_grad()
        sst_label = torch.cat([input_sst[:,1:].float(),sst_true.float()],1).to(self.device)

        loss = self.loss(sst_pred, sst_label)
        loss.backward()
...
if __name__ == '__main__':
    ...
    np.savez('result70.56432056427002.npz', pred=pred.cpu(), label=label.cpu())
