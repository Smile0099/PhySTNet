    ...
    def forward(self, input, target, teacher_forcing=False, scheduled_sampling_ratio=0, train=True):
        ...
        for t in range(self.configs.pre_seq_length + self.configs.aft_seq_length - 1):
            ...
            for i in range(1, self.num_layers):
                ...
                if t==0:
                    h_t[i], c_t[i], st_memory = self.stlstm_layer[i](h_t[i - 1], hidden_state_diff[i-1], h_t[i], c_t[i], st_memory,new_seq=True)
                else:
                    h_t[i], c_t[i], st_memory = self.stlstm_layer[i](h_t[i - 1], hidden_state_diff[i-1], h_t[i], c_t[i], st_memory,new_seq=False)
...
if __name__ == '__main__':
    ...
    model1 = MIM_Model(configs).to(configs.device)
