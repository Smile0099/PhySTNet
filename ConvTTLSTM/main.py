...
class Trainer:
    ...
    def train_once(self, input_sst, sst_true, ssr_ratio):
        sst_pred = self.network(input_sst.float(), sst_true.float().to(self.device), teacher_forcing=True,
                                           scheduled_sampling_ratio=ssr_ratio, train=True)
        self.opt.zero_grad()
        sst_label = sst_true
        loss = self.loss(sst_pred.float().to(self.device), sst_label.float().to(self.device))
        loss.backward()
    ...
    def infer(self, dataset, dataloader):
        ...
        with torch.no_grad():
            ...
            loss_new2 = self.eva_loss(sst_pred, sst_true, self.new2).item()
            loss_new3 = self.eva_loss(sst_pred, sst_true, self.new3).item()
...
if __name__ == '__main__':
    ...
    test_x = np.concatenate([test_x, test_y], 1)
    ...
    np.savez('result84.29003524780273.npz', pred=pred.cpu(), label=label.cpu())
